...
func createSwipeTrail(from: Vector3, to: Vector3, arc: float, duration: float) -> void:
	...
	trail.size_curve = preload("./SkillStealTimeTrailCurve.tres")
	trail.color_gradient = preload("./SkillStealTimeTrailGradient.tres")
	trail.time_curve = preload("./SkillStealTimeTrailTimeCurve.tres")
	trail.time_color_gradient = preload("./SkillStealTimeTrailTimeGradient.tres")
	add_child(trail)
